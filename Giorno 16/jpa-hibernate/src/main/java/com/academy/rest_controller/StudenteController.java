...
//    }
    ...
    @GetMapping("/{id}/esami")
    public List<Esame> getEsamiStudente(@PathVariable Long id) {
        return studenteService.getEsamiByStudente(id);
    }

    @PostMapping("/{id}/esami")
    public Esame addEsame(@PathVariable Long id, @Valid @RequestBody Esame esame) {
        return studenteService.addEsame(id, esame);
    }

    @GetMapping("/{id}/media")
    public Map<String, Double> getMediaStudente(@PathVariable Long id) {
        double media = studenteService.calcolaMedia(id);
